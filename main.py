                ...
                if forwarded and forwarded[2] == reply_id:
                    ...
                    if isinstance(source, discord.TextChannel):
                        try:
                            ...
                            logger.exception("ส่งคำตอบกลับห้องต้นทางไม่สำเร็จ")
                            await message.reply(
                                "ส่งคำตอบกลับไม่สำเร็จ โปรดตรวจสิทธิ์ Send Messages ในห้องต้นทาง",
            ...
            if message.channel.id == source_id:
                ...
                if isinstance(reply_room, discord.TextChannel):
                    ...
                    try:
                        ...
                        logger.exception("ส่งข้อความไปห้องตอบกลับไม่สำเร็จ")
                    else:
                        forwarded_messages[sent.id] = (source_id, message.author.id, reply_id)
                        if len(forwarded_messages) > 10000:
                            oldest_id = next(iter(forwarded_messages))
                            forwarded_messages.pop(oldest_id, None)
                return
